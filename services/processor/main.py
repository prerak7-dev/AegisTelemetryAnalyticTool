...
class WindowState:
    ...
    def update(self, event: dict[str, Any]) -> None:
        ...
        if event_type == "aoe_ability_cast" or str(event.get("ability_id", "")).startswith("aoe_"):
            self.aoe_events += 1
    ...
    def to_row(self) -> tuple:
        ...
        return (
            ...
            self.build_version,
            self.events,
            int(m["active_players"]),
            self.ability_casts,
            self.aoe_events,
            self.physics_event_count,
            float(m["replicated_p95"]),
            float(m["cpu_p95"]),
            float(m["avg_frame"]),
            float(m["p95_frame"]),
            float(m["p99_frame"]),
            float(m["packet_loss_p95"]),
            self.desync_events,
            self.rubberband_events,
            self.hot_risk(),
        )
    ...
    def incident(self) -> dict[str, Any] | None:
        ...
        if hot_risk < 65 and float(m["p95_frame"]) < 50 and float(m["packet_loss_p95"]) < 5:
            return None

        ranked_drivers = score_drivers(m)
        top = top_recommendation(m)
        ...
        evidence = {
            ...
            "ranked_driver_scores": ranked_drivers,
        }
        ...
        return {
            ...
            "symptom": f"p95 server frame {float(m['p95_frame']):.1f} ms, p99 {float(m['p99_frame']):.1f} ms, hot-zone risk {hot_risk:.1f}",
            "likely_driver": top["driver"],
            "confidence": float(top["confidence"]),
            "player_impact": "Potential rubberbanding, delayed hit registration, or match instability.",
            "recommended_action": top["recommendation"],
...
def raw_event_row(event: dict[str, Any]) -> tuple:
    ...
    return (
        ...
        int(event.get("player_count") or 0),
        int(event.get("players_nearby") or 0),
        event.get("ability_id", ""),
        float(event.get("cpu_percent") or 0),
        float(event.get("memory_mb") or 0),
        float(event.get("server_frame_ms") or 0),
        float(event.get("packet_loss_percent") or 0),
        float(event.get("packet_out_kbps") or 0),
        int(event.get("desync_count") or 0),
        int(event.get("rubberband_count") or 0),
        int(event.get("replicated_objects") or 0),
        int(event.get("physics_events") or 0),
        json.dumps(event),
    )
...
def main() -> None:
    ...
    while True:
        ...
        if should_flush:
            ...
            flushable_keys = [
                key for key, state in windows.items()
                if state.window_end + timedelta(seconds=WINDOW_GRACE_SECONDS) <= now
            ]
